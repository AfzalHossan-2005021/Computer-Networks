 ...
 *
 * Based on
 *      NS-2 AODV model developezd by the CMU/MONARCH group and optimized and
...
RreqHeader::Print(std::ostream& os) const
{
    os << "RREQ ID " << m_requestID << " destination: ipv4 " << m_dst << " sequence number "
       ...
       << " flags:" << " Gratuitous RREP " << (*this).GetGratuitousRrep() << " Destination only "
...
{
    os << "RevRREQ ID " << m_requestID << " destination: ipv4 " << m_dst << " sequence number "
       << m_dstSeqNo << " source: ipv4 " << m_origin << " flags:" << " Gratuitous RREP "
       << (*this).GetGratuitousRrep() << " Destination only " << (*this).GetDestinationOnly()
