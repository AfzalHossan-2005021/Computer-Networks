...
RoutingProtocol::RoutingProtocol()
    : m_rreqRetries(2),
      ...
      m_revRreqRateLimitTimer(Timer::CANCEL_ON_DESTROY),
    ...
    static TypeId tid =
        TypeId("ns3::raodv::RoutingProtocol")
            ...
            .AddAttribute("RreqRateLimit",
                          ...
                          MakeUintegerChecker<uint32_t>())
            .AddAttribute("RevRreqRateLimit",
                          "Maximum number of RevRREQ per second.",
                          UintegerValue(10),
                          MakeUintegerAccessor(&RoutingProtocol::m_revRreqRateLimit),
                          MakeUintegerChecker<uint32_t>())
...
RoutingProtocol::Start()
{
    ...
    m_revRreqRateLimitTimer.SetFunction(&RoutingProtocol::RevRreqRateLimitTimerExpire, this);
    m_revRreqRateLimitTimer.Schedule(Seconds(1));
...
void
RoutingProtocol::SendReverseRequest(const RreqHeader& rreqHeader)
{
    Ipv4Address dst = rreqHeader.GetOrigin();
    NS_LOG_FUNCTION(this << dst);
    // A node SHOULD NOT originate more than RREQ_RATELIMIT RREQ messages per second.
    if (m_revRreqCount == m_revRreqRateLimit)
    {
        Simulator::Schedule(m_rreqRateLimitTimer.GetDelayLeft() + MicroSeconds(100),
                            &RoutingProtocol::SendRequest,
                            this,
    ...
    if (m_routingTable.LookupRoute(dst, rt))
    {
        ...
        if (rt.GetValidSeqNo())
        {
            revRreqHeader.SetDstSeqno(rt.GetSeqNo());
        }
        else
        {
            revRreqHeader.SetUnknownSeqno(true);
        }
        rt.SetHop(ttl);
        rt.SetFlag(IN_SEARCH);
        rt.SetLifeTime(m_pathDiscoveryTime);
    ...
    if (m_gratuitousReply)
    {
        revRreqHeader.SetGratuitousRrep(true);
    }
    if (m_destinationOnly)
    {
    ...
    m_seqNo++;
    revRreqHeader.SetOriginSeqno(m_seqNo);
    m_requestId++;
    revRreqHeader.SetId(m_requestId);
    ...
    for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
    {
        ...
        m_revRreqIdCache.IsDuplicate(iface.GetLocal(), m_requestId);
        ...
        TypeHeader tHeader(RAODVTYPE_REV_RREQ);
        packet->AddHeader(tHeader);
        // Send to all-hosts broadcast if on /32 addr, subnet-directed otherwise
        Ipv4Address destination;
        if (iface.GetMask() == Ipv4Mask::GetOnes())
    ...
    // ScheduleRevRreqRetry(rreqHeader);
}

void
RoutingProtocol::ScheduleRevRreqRetry(const RreqHeader& rreqHeader)
{
    Ipv4Address dst = rreqHeader.GetOrigin();
    NS_LOG_FUNCTION(this << dst);
    RoutingTableEntry rt;
    ...
    m_addressRevReqTimer[dst].Schedule(retry);
    NS_LOG_LOGIC("Scheduled RREQ retry in " << retry.As(Time::S));
}

void
RoutingProtocol::RecvRevRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
    NS_LOG_FUNCTION(this);
    RreqHeader rreqHeader;
    p->RemoveHeader(rreqHeader);
    ...
    if (IsMyOwnAddress(rreqHeader.GetDst()))
    {
        m_routingTable.LookupRoute(origin, toOrigin);
        NS_LOG_DEBUG("Send reply since I am the destination");
        SendPacketFromQueue(origin, toOrigin.GetRoute());
        return;
    }
    /*
     * (ii) or it has an active route to the destination, the destination sequence number in the
     ...
     * RREQ.
     */
    if (m_revRreqIdCache.IsDuplicate(origin, id))
    {
        NS_LOG_DEBUG("Ignoring RREQ due to duplicate");
        return;
    }
...
void
RoutingProtocol::RouteRevRequestTimerExpire(Ipv4Address dst)
{
    NS_LOG_LOGIC(this);
    RoutingTableEntry toDst;
    if (m_routingTable.LookupValidRoute(dst, toDst))
...
void
RoutingProtocol::RevRreqRateLimitTimerExpire()
{
    NS_LOG_FUNCTION(this);
    m_revRreqCount = 0;
    m_revRreqRateLimitTimer.Schedule(Seconds(1));
