...
class TcpNewReno : public TcpCongestionOps
{
  ...
  protected:
    ...
    uint32_t GetEstimatedCwnd(Ptr<const TcpSocketState> state);

  private:
    Time m_sumRtt; //!< Sum of the last RTTs
    Time m_lastRtt; //!< Last RTT
    uint32_t m_cntRtt; //!< Number of RTTs in the sum
};
