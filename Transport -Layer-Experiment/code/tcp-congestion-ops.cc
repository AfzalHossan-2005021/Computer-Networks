...
void
TcpNewReno::PktsAcked(Ptr<TcpSocketState> tcb, uint32_t segmentsAcked, const Time& rtt)
{
    NS_LOG_FUNCTION(this << tcb << segmentsAcked << rtt);
...
uint32_t
TcpNewReno::GetEstimatedCwnd(Ptr<const TcpSocketState> state)
{
    Time RTTavg = m_sumRtt / m_cntRtt;
    Time RTTn = m_lastRtt;
    Time deltaRTT = RTTn - RTTavg;
    uint32_t factor = state->m_cWnd / RTTavg.GetSeconds();
    uint32_t avgNum = factor * deltaRTT.GetSeconds();
    return state->m_cWnd - avgNum;
}
...
TcpNewReno::GetSsThresh(Ptr<const TcpSocketState> state, uint32_t bytesInFlight)
{
    ...
    uint32_t tmpCwnd = 2 * state->m_segmentSize;

    if(state->m_congState == TcpSocketState::CA_RECOVERY){
        uint32_t estimatedCwnd = GetEstimatedCwnd(state);
        tmpCwnd = std::max(tmpCwnd, estimatedCwnd);
    }

    return std::max(tmpCwnd, bytesInFlight / 2);
}
